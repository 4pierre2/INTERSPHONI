...
mag = 8.9
rad_pix = 21
coadd_1_name = '/media/pierre/Disque_2/SNR/SORTED_COADD/NGC1068/H+K/0.025/NGC1068-25mas-HK-1_COADD_OBJ_53653.21416709.fits'
...
# obj1 = get_obj(coadd_1_name)
# obj2 = get_obj(coadd_2_name)
# obj3 = get_obj(coadd_3_name)
# obj4 = get_obj(coadd_4_name)[:,:-8,:]
# x1, y1 = np.unravel_index(np.argmax(np.median(obj1,0)), np.shape(obj1)[1:])
# x2, y2 = np.unravel_index(np.argmax(np.median(obj2,0)), np.shape(obj1)[1:])
# x3, y3 = np.unravel_index(np.argmax(np.median(obj3,0)), np.shape(obj1)[1:])
# x4, y4 = np.unravel_index(np.argmax(np.median(obj4,0)), np.shape(obj1)[1:])
# obj2 = np.roll(obj2,(x1-x2,y1-y2), axis=(1,2))
# obj3 = np.roll(obj3,(x1-x3,y1-y3), axis=(1,2))
# obj4 = np.roll(obj4,(x1-x4,y1-y4), axis=(1,2))
# obj = np.mean([obj1,obj2,obj4],0)
...
nod_sinf_0 *= 2.4/0.52
sphobj.plot_spec(-0.05,0.05)
lsph, nod_sph_0, nod_sph_0_err = sphobj.make_spec(-0.05,0.05)
...
plt.figure()
plt.errorbar(lk, knod1, kerr1, label='SINFONI')
plt.errorbar(lh, hnod1, herr1, label='SPHERE+SINFONI')
plt.errorbar(lj, jnod1, jerr1, label='SPHERE')
plt.xlabel('Wavelength (nm)', fontsize=11)
plt.ylabel('Flux ($W.m^{-2}.\mu m^{-1}$)', fontsize=11)
plt.legend()

plt.figure()
plt.errorbar(lk, knod2, kerr2, label='SINFONI')
plt.errorbar(lh, hnod2, herr2, label='SPHERE+SINFONI')
plt.errorbar(lj, jnod2, jerr2, label='SPHERE')
plt.xlabel('Wavelength (nm)', fontsize=11)
plt.ylabel('Flux ($W.m^{-2}.\mu m^{-1}$)', fontsize=11)
plt.legend()

plt.figure()
plt.errorbar(lk, knod0, kerr0, label='SINFONI')
plt.errorbar(lh, hnod0, herr0, label='SPHERE+SINFONI')
plt.errorbar(lj, jnod0, jerr0, label='SPHERE')
plt.xlabel('Wavelength (nm)', fontsize=11)
plt.ylabel('Flux ($W.m^{-2}.\mu m^{-1}$)', fontsize=11)
plt.legend()

fig, (ax0, ax1, ax2) = plt.subplots(3, 1, sharex=True, figsize=(10,5))
ax1.errorbar(lk, knod1, kerr1, label='SINFONI')
ax1.errorbar(lh, hnod1, herr1, label='SPHERE+SINFONI')
ax1.errorbar(lj, jnod1, jerr1, label='SPHERE')
# ax1.xlabel('Wavelength (nm)', fontsize=11)
ax1.set_ylabel('Flux ($W.m^{-2}.\mu m^{-1}$)', fontsize=11)
ax2.errorbar(lk, knod2, kerr2, label='SINFONI')
ax2.errorbar(lh, hnod2, herr2, label='SPHERE+SINFONI')
ax2.errorbar(lj, jnod2, jerr2, label='SPHERE')
# plt.xlabel('Wavelength (nm)', fontsize=11)
# ax2.set_ylabel('Flux ($W.m^{-2}.\mu m^{-1}$)', fontsize=11)
ax0.errorbar(lk, knod0, kerr0, label='SINFONI')
ax0.errorbar(lh, hnod0, herr0, label='SPHERE+SINFONI')
ax0.errorbar(lj, jnod0, jerr0, label='SPHERE')
ax2.set_xlabel('Wavelength (nm)', fontsize=11)
# ax0.set_ylabel('Flux ($W.m^{-2}.\mu m^{-1}$)', fontsize=11)
plt.tight_layout()
